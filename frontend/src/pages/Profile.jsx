...
export default function Profile() {
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 'vault') {
        const res = await axios.get('https://bytesquish.isharankumar.com/api/images/history');
        setImages(res.data);
      } else {
        const res = await axios.get('https://bytesquish.isharankumar.com/api/auth/sessions');
  ...
  const handleRevokeSession = async (id) => {
    try {
      await axios.delete(`https://bytesquish.isharankumar.com/api/auth/sessions/${id}`);
  ...
  const handleForgotPassword = async (e) => {
    ...
    setResetError('');
    try {
      const res = await axios.post('https://bytesquish.isharankumar.com/api/auth/forgot-password', { email: resetEmail });
                  ...
                  <div key={img.id} className={`bg-zinc-950 border p-5 rounded-xl transition-all ${isExpired ? 'border-red-900/50 opacity-50' : 'border-zinc-800 hover:border-sky-500/30'}`}>
                    <div className="flex justify-between items-start mb-4">
                      ...
                      {isExpired ? (
                        ...
                        <a 
                          href={`https://bytesquish.isharankumar.com/api/images/download/${img.id}`} 
