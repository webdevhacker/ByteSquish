...
export default function Profile() {
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === 'vault') {
        const res = await axios.get('http://127.0.0.1:5000/api/images/history');
        setImages(res.data);
      } else {
  ...
  const handleRevokeSession = async (id) => {
    try {
      await axios.delete(`http://127.0.0.1:5000/api/auth/sessions/${id}`);
  ...
  const handleForgotPassword = async (e) => {
    ...
    setResetError('');
    try {
      const res = await axios.post('http://127.0.0.1:5000/api/auth/forgot-password', { email: resetEmail });
                  ...
                  <div key={img.id} className={`bg-zinc-950 border p-5 rounded-xl transition-all ${isExpired ? 'border-red-900/50 opacity-50' : 'border-zinc-800 hover:border-sky-500/30'}`}>
                    <div className="flex justify-between items-start mb-4">
                      ...
                      {isExpired ? (
                        ...
                        <a 
                          href={`http://127.0.0.1:5000/api/images/download/${img.id}`} 
